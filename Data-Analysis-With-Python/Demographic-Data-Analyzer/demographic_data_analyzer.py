...
def calculate_demographic_data(print_data=True):
    ...
    df_max = df.groupby(["native-country", "salary"], group_keys=False).count().rename(columns={"age": "count"}).groupby(level=0, group_keys=False).apply(lambda x:  100*x / x.sum()).sort_values(['count'],ascending=False).groupby(["salary"], group_keys=False).head(1)
    df_max = df_max.reset_index()
    highest_earning_country = df_max[df_max.salary == ">50K"]["native-country"].iloc[0]
    highest_earning_country_percentage = round(df_max[(df_max["native-country"] == highest_earning_country) & (df_max.salary == ">50K")]["count"].iloc[0], 1)
